...
class FortunecookieValidationException implements Exception {
  final List<String> errors;

  FortunecookieValidationException(this.errors);

  @override
  String toString() => 'FortunecookieValidationException: ${errors.join("; ")}';
}
...
/// Fortune Cookie Generator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/fortunecookie?utm_source=dart&utm_medium=readme
///
/// Parameters:

class FortunecookieClient {
  final String apiKey;
  ...
  FortunecookieClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/fortunecookie',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [FortunecookieValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw FortunecookieValidationException(errors);
    }
  }

  /// Execute a request to the Fortune Cookie Generator API.
  ///
  /// Parameters are validated before sending the request.
  Future<FortunecookieResponse> execute() async {
    if (apiKey.isEmpty) {
      throw FortunecookieException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return FortunecookieResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw FortunecookieException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw FortunecookieException('Resource not found');
      } else {
        throw FortunecookieException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is FortunecookieException) rethrow;
      throw FortunecookieException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Fortune Cookie Generator API client.
class FortunecookieException implements Exception {
  final String message;

  FortunecookieException(this.message);

  @override
  String toString() => 'FortunecookieException: $message';
}
